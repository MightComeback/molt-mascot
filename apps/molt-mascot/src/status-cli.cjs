...
function resolveStatusConfig({
  ...
  const { SIZE_PRESETS, DEFAULT_SIZE_INDEX, isValidSize, findSizePreset } =
    sizePresets;

  const resolvedAlign =
    env.MOLT_MASCOT_ALIGN || prefs.alignment || "bottom-right";
  ...
  const resolvedSize = (() => {
    ...
    if (typeof prefs.size === "string" && prefs.size && isValidSize(prefs.size))
      return prefs.size.toLowerCase();
    if (
      typeof prefs.sizeIndex === "number" &&
      prefs.sizeIndex >= 0 &&
      prefs.sizeIndex < SIZE_PRESETS.length
    ) {
  ...
  const resolvedOpacityNum = (() => {
    const envVal = parseEnvNumber(env, "MOLT_MASCOT_OPACITY", -1, {
      min: 0,
      max: 1,
    });
    ...
    if (
      typeof prefs.opacityIndex === "number" &&
      prefs.opacityIndex >= 0 &&
      prefs.opacityIndex < opacityCycle.length
    ) {
  ...
  const resolvedPaddingNum = (() => {
    ...
    if (envVal >= 0) return envVal;
    if (typeof prefs.padding === "number" && prefs.padding >= 0)
      return prefs.padding;
    return 24;
  })();

  const clickThrough = parseEnvBoolean(
    env,
    ["MOLT_MASCOT_CLICK_THROUGH", "MOLT_MASCOT_CLICKTHROUGH"],
    prefs.clickThrough || false,
  );
  const hideText = parseEnvBoolean(
    env,
    ["MOLT_MASCOT_HIDETEXT", "MOLT_MASCOT_HIDE_TEXT"],
    prefs.hideText || false,
  );
  const reducedMotion = parseEnvBoolean(
    env,
    "MOLT_MASCOT_REDUCED_MOTION",
    prefs.reducedMotion || false,
  );
  const startHidden =
    hasBoolFlag("--start-hidden", argv) ||
    parseEnvBoolean(env, "MOLT_MASCOT_START_HIDDEN", false);
  const debug =
    hasBoolFlag("--debug", argv) ||
    parseEnvBoolean(env, "MOLT_MASCOT_DEBUG", false);
  const disableGpu =
    hasBoolFlag("--disable-gpu", argv) ||
    parseEnvBoolean(env, "MOLT_MASCOT_DISABLE_GPU", false);
  const noTray =
    hasBoolFlag("--no-tray", argv) ||
    parseEnvBoolean(env, "MOLT_MASCOT_NO_TRAY", false);
  const noShortcuts =
    hasBoolFlag("--no-shortcuts", argv) ||
    parseEnvBoolean(env, "MOLT_MASCOT_NO_SHORTCUTS", false);
  ...
  const sleepThresholdS = timingPref(
    "MOLT_MASCOT_SLEEP_THRESHOLD_S",
    "sleepThresholdS",
    120,
    { min: 0 },
  );
  const idleDelayMs = timingPref(
    "MOLT_MASCOT_IDLE_DELAY_MS",
    "idleDelayMs",
    800,
    { min: 0 },
  );
  const errorHoldMs = timingPref(
    "MOLT_MASCOT_ERROR_HOLD_MS",
    "errorHoldMs",
    5000,
    { min: 0 },
  );
  ...
  const reconnectMaxMs = timingPref(
    "MOLT_MASCOT_RECONNECT_MAX_MS",
    "reconnectMaxMs",
    30000,
    { min: 0 },
  );
  const staleConnectionMs = timingPref(
    "MOLT_MASCOT_STALE_CONNECTION_MS",
    "staleConnectionMs",
    15000,
    { min: 0 },
  );
  const staleCheckIntervalMs = timingPref(
    "MOLT_MASCOT_STALE_CHECK_INTERVAL_MS",
    "staleCheckIntervalMs",
    5000,
    { min: 0 },
  );
  const pollIntervalMs = timingPref(
    "MOLT_MASCOT_POLL_INTERVAL_MS",
    "pollIntervalMs",
    1000,
    { min: 100 },
  );

  const resolvedSizePreset =
    findSizePreset(resolvedSize) || SIZE_PRESETS[DEFAULT_SIZE_INDEX];
  const resolvedWidth = parseEnvNumber(
    env,
    "MOLT_MASCOT_WIDTH",
    resolvedSizePreset.width,
    { min: 1 },
  );
  const resolvedHeight = parseEnvNumber(
    env,
    "MOLT_MASCOT_HEIGHT",
    resolvedSizePreset.height,
    { min: 1 },
  );
  ...
  const envOverrides = ENV_OVERRIDES_MAP.filter(
  ...
  if (typeof status.uptime === "number") {
    ...
    const startedAt =
      typeof status.startedAt === "number"
        ? ` (since ${formatTimestampLocal(status.startedAt)})`
        : "";
