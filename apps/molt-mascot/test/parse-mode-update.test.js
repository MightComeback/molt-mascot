...
} from "../src/parse-mode-update.cjs";
...
describe("parse-mode-update", () => {
  ...
  describe("parseModeUpdate", () => {
    it("parses a full valid payload", () => {
      ...
      expect(result.latencyStats).toEqual({
        min: 30,
        max: 80,
        avg: 45,
        median: 38,
    ...
    it("parses latencyTrend values", () => {
      expect(parseModeUpdate({ latencyTrend: "rising" }).latencyTrend).toBe(
        "rising",
      );
      expect(parseModeUpdate({ latencyTrend: "falling" }).latencyTrend).toBe(
        "falling",
      );
      expect(parseModeUpdate({ latencyTrend: "stable" }).latencyTrend).toBe(
        "stable",
      );
      expect(
        parseModeUpdate({ latencyTrend: "unknown" }).latencyTrend,
      ).toBeNull();
    ...
    it("rejects latencyStats without numeric samples field", () => {
      expect(
        parseModeUpdate({ latencyStats: { median: 38 } }).latencyStats,
      ).toBeNull();
      expect(
        parseModeUpdate({ latencyStats: "not an object" }).latencyStats,
      ).toBeNull();
    ...
    it("accepts latencyStats with numeric samples field", () => {
      ...
      expect(parseModeUpdate({ latencyStats: stats }).latencyStats).toEqual(
        stats,
      );
    });
    ...
    it("rejects latencyStats with non-integer samples", () => {
      expect(
        parseModeUpdate({ latencyStats: { samples: 1.5 } }).latencyStats,
      ).toBeNull();
    });

    it("rejects latencyStats with zero samples", () => {
      expect(
        parseModeUpdate({ latencyStats: { samples: 0 } }).latencyStats,
      ).toBeNull();
    });

    it("rejects latencyStats with NaN/Infinity in numeric fields", () => {
      expect(
        parseModeUpdate({ latencyStats: { samples: 5, min: NaN } })
          .latencyStats,
      ).toBeNull();
      expect(
        parseModeUpdate({ latencyStats: { samples: 5, max: Infinity } })
          .latencyStats,
      ).toBeNull();
      expect(
        parseModeUpdate({ latencyStats: { samples: 5, avg: -Infinity } })
          .latencyStats,
      ).toBeNull();
    });
    ...
    it("rejects latencyStats with negative numeric fields", () => {
      expect(
        parseModeUpdate({ latencyStats: { samples: 5, median: -1 } })
          .latencyStats,
      ).toBeNull();
      expect(
        parseModeUpdate({ latencyStats: { samples: 5, p95: -10 } })
          .latencyStats,
      ).toBeNull();
      expect(
        parseModeUpdate({ latencyStats: { samples: 5, jitter: -5 } })
          .latencyStats,
      ).toBeNull();
    });
    ...
    it("accepts latencyMs as an alias for latency", () => {
      ...
      expect(parseModeUpdate({ latencyMs: 55, latency: 99 }).latencyMs).toBe(
        55,
      );
    ...
    it("parses connectionSuccessRate as integer percentage 0-100", () => {
      expect(
        parseModeUpdate({ connectionSuccessRate: 95 }).connectionSuccessRate,
      ).toBe(95);
      expect(
        parseModeUpdate({ connectionSuccessRate: 0 }).connectionSuccessRate,
      ).toBe(0);
      expect(
        parseModeUpdate({ connectionSuccessRate: 100 }).connectionSuccessRate,
      ).toBe(100);
      // rejects out-of-range
      expect(
        parseModeUpdate({ connectionSuccessRate: 101 }).connectionSuccessRate,
      ).toBeNull();
      expect(
        parseModeUpdate({ connectionSuccessRate: -1 }).connectionSuccessRate,
      ).toBeNull();
      // rejects non-integer
      expect(
        parseModeUpdate({ connectionSuccessRate: 95.5 }).connectionSuccessRate,
      ).toBeNull();
      // rejects non-number
      expect(
        parseModeUpdate({ connectionSuccessRate: "95" }).connectionSuccessRate,
      ...
      expect(
        parseModeUpdate({ connectionSuccessRate: null }).connectionSuccessRate,
      ).toBeNull();
    });

    it("parses connectionUptimePct as integer percentage 0-100", () => {
      expect(
        parseModeUpdate({ connectionUptimePct: 85 }).connectionUptimePct,
      ).toBe(85);
      expect(
        parseModeUpdate({ connectionUptimePct: 0 }).connectionUptimePct,
      ).toBe(0);
      expect(
        parseModeUpdate({ connectionUptimePct: 100 }).connectionUptimePct,
      ).toBe(100);
      // rejects out-of-range
      expect(
        parseModeUpdate({ connectionUptimePct: 101 }).connectionUptimePct,
      ).toBeNull();
      expect(
        parseModeUpdate({ connectionUptimePct: -1 }).connectionUptimePct,
      ).toBeNull();
      // rejects non-integer
      expect(
        parseModeUpdate({ connectionUptimePct: 85.5 }).connectionUptimePct,
      ).toBeNull();
      // rejects non-number
      expect(
        parseModeUpdate({ connectionUptimePct: "85" }).connectionUptimePct,
      ...
      expect(
        parseModeUpdate({ connectionUptimePct: null }).connectionUptimePct,
      ).toBeNull();
    });
  ...
  describe("formatModeUpdate", () => {
    ...
    it("includes closeDetail when present", () => {
      ...
      expect(formatModeUpdate(parsed)).toContain(
        'close="abnormal closure (1006)"',
      );
    });
    ...
    it("includes latencyTrend arrow when rising or falling", () => {
      expect(
        formatModeUpdate(parseModeUpdate({ latencyTrend: "rising" })),
      ).toContain("↑");
      expect(
        formatModeUpdate(parseModeUpdate({ latencyTrend: "falling" })),
      ).toContain("↓");
    });

    it("omits latencyTrend when stable", () => {
      const str = formatModeUpdate(
        parseModeUpdate({ mode: "idle", latencyTrend: "stable" }),
      );
