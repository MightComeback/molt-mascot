...
describe("tray-icon", () => {
  describe("TRAY_SPRITE", () => {
    ...
    it("contains at least one non-transparent pixel", () => {
      const hasColor = TRAY_SPRITE.some((row) =>
        [...row].some((ch) => ch !== "."),
      );
  ...
  describe("buildTrayTooltip", () => {
    ...
    it("shows mode emoji for non-idle modes", () => {
      expect(buildTrayTooltip({ ...base, mode: "thinking" })).toContain(
        "🧠 thinking",
      );
      expect(buildTrayTooltip({ ...base, mode: "tool" })).toContain("🔧 tool");
      expect(
        buildTrayTooltip({ ...base, mode: "tool", currentTool: "web_search" }),
      ).toContain("🔧 web_search");
      expect(
        buildTrayTooltip({ ...base, mode: "tool", currentTool: "web_search" }),
      ).not.toContain("🔧 tool");
      expect(buildTrayTooltip({ ...base, mode: "error" })).toContain(
        "❌ error",
      );
      expect(buildTrayTooltip({ ...base, mode: "sleeping" })).toContain(
        "💤 sleeping",
      );
      expect(buildTrayTooltip({ ...base, mode: "disconnected" })).toContain(
        "⚡ disconnected",
      );
    });
    ...
    it("omits median when latencyStats has only 1 sample", () => {
      const tip = buildTrayTooltip({
        ...base,
        latencyMs: 42,
        latencyStats: { min: 42, max: 42, avg: 42, median: 42, samples: 1 },
      });
      expect(tip).toContain("⏱ 42ms");
    ...
    it("omits median when latencyStats has no median field", () => {
      const tip = buildTrayTooltip({
        ...base,
        latencyMs: 42,
        latencyStats: { min: 30, max: 80, avg: 45, samples: 5 },
      });
      expect(tip).toContain("⏱ 42ms");
      expect(tip).not.toContain("(med");
    });
    ...
    it("omits jitter when jitter field is not present", () => {
      const tip = buildTrayTooltip({
        ...base,
        latencyMs: 42,
        latencyStats: { min: 30, max: 80, avg: 45, median: 38, samples: 20 },
      });
      expect(tip).not.toContain("jitter");
    });
    ...
    it("omits mode duration when zero or undefined", () => {
      expect(
        buildTrayTooltip({ ...base, mode: "tool", modeDurationSec: 0 }),
      ).not.toContain("(0s)");
    ...
    it("omits started-at suffix when processStartedAt is zero or negative", () => {
      expect(
        buildTrayTooltip({ ...base, processUptimeS: 60, processStartedAt: 0 }),
      ).not.toContain("(since ");
      expect(
        buildTrayTooltip({ ...base, processUptimeS: 60, processStartedAt: -1 }),
      ).not.toContain("(since ");
    });
    ...
    it("shows process memory RSS when processMemoryRssBytes is provided", () => {
      const tip = buildTrayTooltip({
        ...base,
        processMemoryRssBytes: 52428800,
      }); // 50 MB
    ...
    it("omits process memory when zero or negative", () => {
      expect(
        buildTrayTooltip({ ...base, processMemoryRssBytes: 0 }),
      ).not.toContain("🧠");
      expect(
        buildTrayTooltip({ ...base, processMemoryRssBytes: -1 }),
      ).not.toContain("🧠");
    });
    ...
    it("omits reconnect count when sessionConnectCount is 0 or 1", () => {
      expect(
        buildTrayTooltip({ ...base, sessionConnectCount: 0 }),
      ).not.toContain("↻");
      expect(
        buildTrayTooltip({ ...base, sessionConnectCount: 1 }),
      ).not.toContain("↻");
      expect(buildTrayTooltip({ ...base })).not.toContain("↻");
    });
    ...
    it("omits active agents/tools when both are zero", () => {
      expect(
        buildTrayTooltip({ ...base, activeAgents: 0, activeTools: 0 }),
      ).not.toContain("agent");
    ...
    it("omits last reset when lastResetAt is not provided or zero", () => {
      ...
      expect(buildTrayTooltip({ ...base, lastResetAt: 0 })).not.toContain(
        "🔄 reset",
      );
      expect(buildTrayTooltip({ ...base, lastResetAt: null })).not.toContain(
        "🔄 reset",
      );
    });
    ...
    it("omits last message gap when lastMessageAt is null or 0", () => {
      expect(
        buildTrayTooltip({ ...base, uptimeStr: "5m", lastMessageAt: null }),
      ).not.toContain("📩");
      expect(
        buildTrayTooltip({ ...base, uptimeStr: "5m", lastMessageAt: 0 }),
      ).not.toContain("📩");
    });
    ...
    it("omits close detail when null or empty", () => {
      expect(
        buildTrayTooltip({ ...base, lastCloseDetail: null }),
      ).not.toContain("⚡");
    ...
    it("omits reconnect attempt when 0", () => {
      expect(buildTrayTooltip({ ...base, reconnectAttempt: 0 })).not.toContain(
        "retry",
      );
    });
    ...
    it("omits health status when healthy or not provided", () => {
      expect(
        buildTrayTooltip({ ...base, healthStatus: "healthy" }),
      ).not.toContain("degraded");
      expect(
        buildTrayTooltip({ ...base, healthStatus: "healthy" }),
      ).not.toContain("unhealthy");
